import GSlamContPfBot2DWrapper
import numpy as np
import sac
...
#%%
is_train = True
model_path = {"actor":"models/SAC_Exp_ANet_pf2.pkl", "critic":"models/SAC_Exp_CNet_pf2.pkl"}
seq_size = 3
...
if __name__ == '__main__':
    ...
    for eps in range(501):
        ...
        while True:
            ...
            print('\rEps: {:3d} | Step: {:4d} | R: {:+.3f} | Loss: [A>{:+.3f} C>{:+.3f}] | Alpha: {:.4f} | Action: [{:+.3f} {:+.3f}] | R_tot: {:.3f}'\
                .format(eps, step, reward, loss_a, loss_c, RL.alpha, action[0], action[1], acc_reward), end="")
            if step > 0:
                print(' | pfIg: {:+.3f} | pfPG: {:+.3f}\t'.format(state_next["pf_info_gain"], state_next["pf_pose_gain"]), end='')
