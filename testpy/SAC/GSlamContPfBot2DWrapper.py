...
def SensorMapping(gmap, bot_pos, bot_param, sensor_data, gsize=4):
    ...
    for pts, dist in zip(plist, sensor_data):
        ...
        if dist < 1:
            continue
        elif dist < bot_param['max_dist'] - 2:
...
class Bot2DEnv:
    ...
    def reset(self):
        ...
        self.pf = gslam.ParticleFilter(self.env.pose, self.bot_param, self.map, self.n_particles)
    ...
    def step(self, action):
        action0 = self.bot_param['velocity']*(action[0]+1)
        action1 = self.bot_param['rotate_step']*action[1]
        ...
        bid = self.pf.bestSampleId()
        bp = self.pf.getParticle(bid)
        self.obs = bp.getMap().getObserv((bp.pose[0], bp.pose[1]), bp.pose[2] ,int(fsize/2),int(fsize/2)).reshape([fsize,fsize,1])
        
        self.sensor_data = self.env.scan()
        info_gain = SensorMapping(self.map, self.env.pose, self.bot_param, self.sensor_data['data'], self.grid_size)
        sdata = np.array(self.sensor_data['data'], dtype=np.float32)/self.bot_param['max_dist']
        self.neff = self.pf.contFeed(action0, action1, self.sensor_data)
        if self.neff < 0.5:
            self.pf.resampling()
        ...
        # Particle Expected Estimate
        pf_infoGain = 0
        pf_poseErr = 0
        for i in range(self.n_particles):
            pf_infoGain += self.pf.getInfoGain(i) * self.pf.getWeight(i)
            pf_poseErr += self.pf.getWeight(i) * sqrt(
                (self.pf.getPose(i)[0] - self.env.pose[0])**2 + \
                (self.pf.getPose(i)[1] - self.env.pose[1])**2 + \
                (self.pf.getPose(i)[2] - self.env.pose[2])**2 )
        pf_poseGain = self.pf_poseErr_past - pf_poseErr
        self.pf_poseErr_past = pf_poseErr
        ...
        if self.task == 'Navigation':
            ...
            reward = info_gain + 10*pf_poseGain
            if collision:
                reward += -200
                done = 0.
        ...
        return {"map":self.obs, 
                ...
                "goal":self.getRelPos(), 
                "info_gain":info_gain, 
                "pf_info_gain":pf_infoGain, 
                "pf_pose_gain":pf_poseGain,
                "pf_pose_err":pf_poseErr 
                }, reward, done, 
    ...
    def render(self):
        ...
        bid = self.pf.bestSampleId()
        bp = self.pf.getParticle(bid)
        pf_map = bp.getMap().getWholeMapProb()
        ...
        #for i in range(self.n_particles):
        #    pf_traj = self.pf.getParticle(i).getTraj()
        #    img = DrawPath(img, pf_traj, color=(255,0,255))
        img = DrawPath(img, self.pf.getParticle(bid).getTraj(), color=(255,0,255))
        ...
        pimg_re = cv2.resize(pf_map,(int(pf_map.shape[1]*1),int(pf_map.shape[0]*1)),interpolation=cv2.INTER_CUBIC)
        
        # Show 
        cv2.imshow('env',img_re)
        cv2.imshow('map',mimg_re)
        cv2.imshow('pf_map',pimg_re)
